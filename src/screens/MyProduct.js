...
function MyProduct(props) {
  ...
  const getorders = async (request) => { 
    ...
    console.log(UsId);
    await post('/userprods/userprod-list', {
      user_id: UsId,
      token: token
    }).then(async res => {
  ...
  const addfood = async (fcid) => {
    ...
    await post('/userprods/userprod-eating', {
      user_id:usId,
      token: token,
      user_prod_id: fcid,
      meal_time:meal,
    ...
    await post('/userprods/userprod-delete', {
      user_id:usId,
      token: token,
      user_prod_id: fcid,
      ...
      <View style={styles.RootNavLine}>
        <TouchableOpacity
          style={styles.RootNavLineRow}
           onPress={() => navigation2.navigate('AddProduct')}>
           
          <Text style={styles.RootNavLineText}><MaterialCommunityIconsIcon name='plus' size={16} style={{color: 'black'}} /> Добавить продукт </Text>
        </TouchableOpacity>
      </View>
